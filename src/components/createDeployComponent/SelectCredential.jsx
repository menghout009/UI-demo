...
const credential = ["sophak/******"];

export default function SelectCredential() {
  const [selected, setSelected] = useState(credential[0]);
      ...
      <Listbox value={selected} onChange={setSelected}>
        <div className="relative mt-1">
          <Listbox.Button className="relative w-full cursor-default rounded-lg outline-1 outline  outline-[#E2E8F0] bg-white py-2 pl-3 pr-10 text-left focus:outline-primary focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
            <span className="block truncate text-sm py-0.5 px-3">{selected}</span>
          ...
          >
            <Listbox.Options className="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {credential.map((credential, credentialIdx) => (
                <Listbox.Option
                  key={credentialIdx}
                  className={({ active }) =>
                    `relative cursor-default select-none py-2 pl-10 pr-4 ${
                      active ? "bg-database-bgColor text-primary" : "text-gray-900"
                    }`
                  }
                  value={credential}
                >
